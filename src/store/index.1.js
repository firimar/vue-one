  ...
  mutations: {
    ...
    getCacheData(state, result) {
      ...
      if (state.cacheData.length > 0) {
        state.cacheData.forEach(function (item) {
    ...
    getToDay(state, result) {
      ...
      let n = result.data.content_list.length - 1
      state.radioSrc = result.data.content_list[n].audio_url
      state.defaultAudios = result.data.content_list[n].default_audios || '#'
    },
    getOneDay(state, result) {
      //state.showDate = result.data.date
      state.oneDay.datas = result.data.content_list
      state.oneDay.menu = result.data.menu
      state.oneDay.content = result.data.content_list[0]
      let n = result.data.content_list.length - 1
      state.radioSrc = result.data.content_list[n].audio_url
      state.defaultAudios = result.data.content_list[n].default_audios || '#'
    },
    getYesterDay(state, result) {
      //state.showDate = result.data.date
      state.yesterDay.datas = result.data.content_list
      state.yesterDay.menu = result.data.menu
      state.yesterDay.content = result.data.content_list[0]
      let n = result.data.content_list.length - 1
      state.radioSrc = result.data.content_list[n].audio_url
      state.defaultAudios = result.data.content_list[n].default_audios || '#'
    },
